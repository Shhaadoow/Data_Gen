...
/* Diccionario clínico */
.dict-card  { background:#0f172a; border:1px solid #1e3a5f; border-radius:8px; padding:14px 18px; margin:10px 0; }
.dict-title { font-family:'JetBrains Mono',monospace; color:#38bdf8; font-size:0.95rem; font-weight:600; margin-bottom:8px; }
</style>
...
def traducir_al_espanol(texto):
    """Traductor automático ligero usando la API pública de MyMemory."""
    try:
        texto_url = urllib.parse.quote(texto)
        url = f"https://api.mymemory.translated.net/get?q={texto_url}&langpair=en|es"
        r = requests.get(url, timeout=5)
        if r.status_code == 200:
            traduccion = r.json().get("responseData", {}).get("translatedText", texto)
...
@st.cache_data(show_spinner=False, ttl=86400)
def consultar_info_variante_ensembl(rsid):
    """Consulta la API de Ensembl para obtener la data estructural del gen."""
    rsid = str(rsid).strip().lower()
    url = f"https://rest.ensembl.org/variation/human/{rsid}?content-type=application/json"
    
    # Diccionario local para traducir las consecuencias genéticas más comunes
    consecuencias_es = {
        "intron_variant": "Variante en intrón (no codificante)",
        "missense_variant": "Variante de cambio de sentido",
        "synonymous_variant": "Variante sinónima",
        "intergenic_variant": "Variante intergénica",
        "non_coding_transcript_exon_variant": "Variante en exón no codificante",
        "regulatory_region_variant": "Variante en región reguladora",
        "3_prime_UTR_variant": "Variante en región 3' UTR",
        "5_prime_UTR_variant": "Variante en región 5' UTR"
    }
    
    try:
        r = requests.get(url, headers={
            "Content-Type": "application/json",
            "User-Agent": "GenomicLens/1.0 (Proyecto academico UTPL; contacto: practicum-gwas@utpl.edu.ec)",
        }, timeout=8)
        if r.status_code != 200:
            return None
            
        data = r.json()
        consecuencia_en = data.get("most_severe_consequence", "Desconocida")
        
        info = {
            "Consecuencia": consecuencias_es.get(consecuencia_en, consecuencia_en.replace("_", " ").capitalize()),
            "Alelos": data.get("mappings", [{}])[0].get("allele_string", "No disponible"),
            "Gen": "Desconocido",
            "Ubicacion": data.get("mappings", [{}])[0].get("location", "No disponible")
        }
        
        # Extraer el gen mapeado (si existe)
        if "synonyms" in data:
            info["Gen"] = data.get("name", rsid)
...
@st.cache_data(show_spinner=False, ttl=86400)
def consultar_snp_ebi(rsid):
    """Consulta la API REST del EBI GWAS Catalog y traduce los resultados."""
    rsid = str(rsid).strip().lower()
    if not rsid.startswith("rs"):
        return None, None, "El ID debe tener el formato estándar 'rs...' (ej. rs11210887)."

    url_gwas = f"https://www.ebi.ac.uk/gwas/rest/api/singleNucleotidePolymorphisms/{rsid}/associations?projection=associationBySnp"
    headers = {
    ...
    try:
        # 1. Obtener datos clínicos de GWAS Catalog
        r = requests.get(url_gwas, headers=headers, timeout=12)
        if r.status_code == 404:
            return None, None, "Variante sin registros clínicos en la base de datos."
        r.raise_for_status()
        ...
        if "_embedded" in data and "associations" in data["_embedded"]:
            for assoc in data["_embedded"]["associations"]:
                if isinstance(assoc.get("efoTraits"), list):
                    for trait_obj in assoc["efoTraits"]:
                        if "trait" in trait_obj: asociaciones_en.append(trait_obj["trait"])
                if assoc.get("diseaseTrait") and "trait" in assoc["diseaseTrait"]:
                    asociaciones_en.append(assoc["diseaseTrait"]["trait"])
        
        rasgos_en_unicos = sorted(list(set(asociaciones_en)))
        
        # Traducir los rasgos al español
        rasgos_es = [traducir_al_espanol(rasgo) for rasgo in rasgos_en_unicos] if rasgos_en_unicos else []
        
        # 2. Obtener datos estructurales de Ensembl
        info_variante = consultar_info_variante_ensembl(rsid)
        
        if rasgos_es or info_variante:
            return rasgos_es, info_variante, None
        else:
            return None, None, "La variante existe, pero no tiene información clínica mapeada."
            
    except Exception as e:
        return None, None, f"Error de conexión con servidores internacionales: {str(e)}"

@st.cache_data(show_spinner=False, ttl=86400)
def buscar_diccionario_medico(termino):
    """
    Busca una explicación sencilla en español para un rasgo/enfermedad clínica
    usando la API pública de Wikipedia (resumen + imagen si existe).
    Devuelve (titulo, texto, imagen_url, error).
    """
    termino = str(termino).strip()
    if not termino:
        return None, None, None, "Término vacío."

    termino_url = urllib.parse.quote(termino.replace(" ", "_"))
    url = f"https://es.wikipedia.org/api/rest_v1/page/summary/{termino_url}"
    headers = {
        "Accept": "application/json",
        "User-Agent": "GenomicLens/1.0 (Proyecto academico UTPL; contacto: practicum-gwas@utpl.edu.ec)",
    }
    ...
    try:
        ...
        if r.status_code == 404:
            return None, None, None, f"No se encontró una entrada en Wikipedia en español para '{termino}'."
        r.raise_for_status()
        data = r.json()

        titulo   = data.get("title", termino)
        extracto = data.get("extract", "Sin descripción disponible.")
        imagen   = data.get("thumbnail", {}).get("source")

        return titulo, extracto, imagen, None
    except Exception as e:
        return None, None, None, f"Error de conexión al buscar '{termino}': {str(e)}"
...
if MODO == "📄 Archivo Individual":
    ...
    # ── NUEVA TAB 10: VALIDACIÓN CLÍNICA, INFO ESTRUCTURAL Y DICCIONARIO ────────────
    with t10:
        st.markdown("### 🌐 Validación Clínica Internacional y Estructura Genómica")
        st.markdown("Verifique las enfermedades asociadas a la variante (GWAS Catalog) y su estructura física en el ADN (Ensembl). Todo traducido al español.")
        
        c_busq, c_info = st.columns([1, 1])
        with c_busq:
            ejemplo_rsid = "rs11210887"
            if ck["snp"] and ck["pval"]:
                df_top_api = df_raw.dropna(subset=[ck["pval"], ck["snp"]])
                if not df_top_api.empty:
                    snp_sugerido = str(df_top_api.loc[df_top_api[ck["pval"]].idxmin(), ck["snp"]])
                    if snp_sugerido.startswith("rs"): ejemplo_rsid = snp_sugerido

            origen_id = st.radio(
                "Origen del ID", ["Elegir de este dataset", "Escribir manualmente"],
                horizontal=True, key="origen_id_individual",
            )

            if origen_id == "Elegir de este dataset" and ck["snp"]:
                ids_disponibles = df_raw[ck["snp"]].dropna().astype(str)
                ids_rs = sorted(ids_disponibles[ids_disponibles.str.lower().str.startswith("rs")].unique())
                if ids_rs:
                    idx_default = ids_rs.index(ejemplo_rsid) if ejemplo_rsid in ids_rs else 0
                    rsid_input = st.selectbox(
                        f"🧬 Selecciona una variante ({len(ids_rs):,} disponibles):",
                        ids_rs, index=idx_default,
                    )
                else:
                    st.markdown(
                        f'<div class="warn-panel">⚠️ La columna <code>{ck["snp"]}</code> '
                        'no contiene IDs en formato rsXXXXXX. Usa "Escribir manualmente".</div>',
                        unsafe_allow_html=True
                    )
                    rsid_input = st.text_input("🧬 Ingrese Variante Genómica (RSID):", value=ejemplo_rsid)
            else:
                rsid_input = st.text_input("🧬 Ingrese Variante Genómica (RSID):", value=ejemplo_rsid)

            buscar_btn = st.button("Consultar Bases de Datos (EBI + Ensembl)", type="primary")

        if buscar_btn and rsid_input:
            with st.spinner("Conectando con GWAS Catalog y Ensembl. Traduciendo resultados..."):
                rasgos, info_var, error = consultar_snp_ebi(rsid_input)
                
            if error and not rasgos and not info_var: 
                st.error(error)
            else:
                # 1. MOSTRAR LA TARJETA DE INFORMACIÓN ESTRUCTURAL (Como en tu imagen)
                if info_var:
                    st.markdown("#### 🧬 Información Estructural de la Variante")
                    st.markdown(f"""
                    <div style="background-color: #0f172a; border: 1px solid #38bdf8; border-radius: 8px; padding: 15px; margin-bottom: 20px;">
                        <div style="display: flex; justify-content: space-between; flex-wrap: wrap;">
                            <div style="flex: 1; min-width: 200px; margin-bottom: 10px;">
                                <span style="color: #94a3b8; font-size: 0.8rem; text-transform: uppercase;">Ubicación Genómica</span><br>
                                <strong style="color: #e2e8f0; font-size: 1.1rem;">{info_var.get('Ubicacion', '—')}</strong>
                            </div>
                            <div style="flex: 1; min-width: 200px; margin-bottom: 10px;">
                                <span style="color: #94a3b8; font-size: 0.8rem; text-transform: uppercase;">Consecuencia más severa</span><br>
                                <strong style="color: #e2e8f0; font-size: 1.1rem;">{info_var.get('Consecuencia', '—')}</strong>
                            </div>
                            <div style="flex: 1; min-width: 200px; margin-bottom: 10px;">
                                <span style="color: #94a3b8; font-size: 0.8rem; text-transform: uppercase;">Alelos (Mutación)</span><br>
                                <strong style="color: #e2e8f0; font-size: 1.1rem;">{info_var.get('Alelos', '—')}</strong>
                            </div>
                        </div>
                    </div>
                    """, unsafe_allow_html=True)

                # 2. MOSTRAR LAS ENFERMEDADES TRADUCIDAS
                if rasgos:
                    st.success(f"✅ Se encontraron **{len(rasgos)}** fenotipos documentados para {rsid_input}:")
                    html_badges = "".join([f'<span style="background-color: #1e40af; color: white; padding: 5px 12px; border-radius: 15px; margin: 4px; display: inline-block; font-size: 0.85rem; border: 1px solid #3b82f6;">{r}</span>' for r in rasgos])
                    st.markdown(f"<div>{html_badges}</div>", unsafe_allow_html=True)
                    st.session_state['rasgos_encontrados_api'] = rasgos
                else:
                    st.info("La estructura genética fue encontrada, pero no tiene enfermedades confirmadas en el catálogo.")

        # 3. DICCIONARIO MÉDICO EN ESPAÑOL
        st.divider()
        st.markdown("### 📖 Traductor y Diccionario Clínico")
        st.caption("Seleccione una de las enfermedades descubiertas arriba para obtener una explicación sencilla en español.")
        
        lista_rasgos = st.session_state.get('rasgos_encontrados_api', [])
        if not lista_rasgos: 
            st.info("Realice una consulta de variante arriba para habilitar los términos en el diccionario.")
        else:
            termino_sel = st.selectbox("Seleccione el concepto clínico a explorar:", ["(Seleccione un término)"] + lista_rasgos)
            if termino_sel != "(Seleccione un término)":
                with st.spinner(f"Buscando explicación médica para '{termino_sel}'..."):
                    tit_w, txt_w, img_w, err_w = buscar_diccionario_medico(termino_sel)
                st.markdown('<div class="dict-card">', unsafe_allow_html=True)
                if err_w: 
                    st.warning(err_w)
                else:
                    st.markdown(f'<div class="dict-title">🔍 Significado de: {tit_w}</div>', unsafe_allow_html=True)
                    col_t, col_i = st.columns([3, 1])
                    with col_t: st.write(txt_w)
                    with col_i:
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # ── BUSCADOR GWAS CATALOG (EBI + Ensembl) — COMPARATIVA ─────────────────
    with c7:
        st.markdown("### 🌐 Validación Clínica Internacional y Estructura Genómica")
        st.markdown(
            "Busca una variante entre los trastornos cargados en esta comparativa "
            "(o escríbela manualmente) y consulta sus enfermedades asociadas (GWAS "
            "Catalog) y estructura física (Ensembl). Todo traducido al español."
        )

        c_busq, c_info = st.columns([1, 1])
        with c_busq:
            trastorno_sel = st.selectbox(
                "Trastorno del que quieres tomar el ID:",
                list(datasets.keys()), key="trastorno_busqueda_comp",
            )
            ck_sel = datasets[trastorno_sel]["ck"]
            df_sel = datasets[trastorno_sel]["df"]

            origen_id_comp = st.radio(
                "Origen del ID", ["Elegir de este trastorno", "Escribir manualmente"],
                horizontal=True, key="origen_id_comp",
            )

            rsid_input_comp = None
            if origen_id_comp == "Elegir de este trastorno" and ck_sel["snp"]:
                ids_disp_comp = df_sel[ck_sel["snp"]].dropna().astype(str)
                ids_rs_comp = sorted(ids_disp_comp[ids_disp_comp.str.lower().str.startswith("rs")].unique())
                if ids_rs_comp:
                    rsid_input_comp = st.selectbox(
                        f"🧬 Selecciona una variante ({len(ids_rs_comp):,} disponibles en {trastorno_sel}):",
                        ids_rs_comp, key="rsid_select_comp",
                    )
                else:
                    st.markdown(
                        f'<div class="warn-panel">⚠️ La columna de ID de <code>{trastorno_sel}</code> '
                        'no contiene IDs en formato rsXXXXXX. Usa "Escribir manualmente".</div>',
                        unsafe_allow_html=True
                    )
                    rsid_input_comp = st.text_input("🧬 Ingrese Variante Genómica (RSID):", key="rsid_text_comp")
            else:
                rsid_input_comp = st.text_input("🧬 Ingrese Variante Genómica (RSID):", key="rsid_text_comp2")

            buscar_btn_comp = st.button("Consultar Bases de Datos (EBI + Ensembl)",
                                          type="primary", key="buscar_btn_comp")

        if buscar_btn_comp and rsid_input_comp:
            with st.spinner("Conectando con GWAS Catalog y Ensembl. Traduciendo resultados..."):
                rasgos_c, info_var_c, error_c = consultar_snp_ebi(rsid_input_comp)

            if error_c and not rasgos_c and not info_var_c:
                st.error(error_c)
            else:
                if info_var_c:
                    st.markdown("#### 🧬 Información Estructural de la Variante")
                    ...
                    <div style="background-color: #0f172a; border: 1px solid #38bdf8; border-radius: 8px; padding: 15px; margin-bottom: 20px;">
                        <div style="display: flex; justify-content: space-between; flex-wrap: wrap;">
                            <div style="flex: 1; min-width: 200px; margin-bottom: 10px;">
                                <span style="color: #94a3b8; font-size: 0.8rem; text-transform: uppercase;">Ubicación Genómica</span><br>
                                <strong style="color: #e2e8f0; font-size: 1.1rem;">{info_var_c.get('Ubicacion', '—')}</strong>
                            </div>
                            <div style="flex: 1; min-width: 200px; margin-bottom: 10px;">
                                <span style="color: #94a3b8; font-size: 0.8rem; text-transform: uppercase;">Consecuencia más severa</span><br>
                                <strong style="color: #e2e8f0; font-size: 1.1rem;">{info_var_c.get('Consecuencia', '—')}</strong>
                            </div>
                            <div style="flex: 1; min-width: 200px; margin-bottom: 10px;">
                                <span style="color: #94a3b8; font-size: 0.8rem; text-transform: uppercase;">Alelos (Mutación)</span><br>
                                <strong style="color: #e2e8f0; font-size: 1.1rem;">{info_var_c.get('Alelos', '—')}</strong>
                            </div>
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
                ...
                if rasgos_c:
                    st.success(f"✅ Se encontraron **{len(rasgos_c)}** fenotipos documentados para {rsid_input_comp}:")
                    html_badges_c = "".join([f'<span style="background-color: #1e40af; color: white; padding: 5px 12px; border-radius: 15px; margin: 4px; display: inline-block; font-size: 0.85rem; border: 1px solid #3b82f6;">{r}</span>' for r in rasgos_c])
                    st.markdown(f"<div>{html_badges_c}</div>", unsafe_allow_html=True)
                    st.session_state['rasgos_encontrados_api_comp'] = rasgos_c
                else:
                    st.info("La estructura genética fue encontrada, pero no tiene enfermedades confirmadas en el catálogo.")

        st.divider()
        st.markdown("### 📖 Traductor y Diccionario Clínico")
        st.caption("Selecciona una de las enfermedades descubiertas arriba para obtener una explicación sencilla en español.")

        lista_rasgos_comp = st.session_state.get('rasgos_encontrados_api_comp', [])
        if not lista_rasgos_comp:
            st.info("Realiza una consulta de variante arriba para habilitar los términos en el diccionario.")
        else:
            termino_sel_comp = st.selectbox(
                "Selecciona el concepto clínico a explorar:",
                ["(Seleccione un término)"] + lista_rasgos_comp, key="termino_sel_comp",
            )
            if termino_sel_comp != "(Seleccione un término)":
                with st.spinner(f"Buscando explicación médica para '{termino_sel_comp}'..."):
                    tit_c, txt_c, img_c, err_c = buscar_diccionario_medico(termino_sel_comp)
                st.markdown('<div class="dict-card">', unsafe_allow_html=True)
                if err_c:
                    st.warning(err_c)
                else:
                    st.markdown(f'<div class="dict-title">🔍 Significado de: {tit_c}</div>', unsafe_allow_html=True)
                    col_t, col_i = st.columns([3, 1])
                    with col_t: st.write(txt_c)
                    with col_i:
                        if img_c: st.image(img_c, use_container_width=True)
                st.markdown('</div>', unsafe_allow_html=True)
